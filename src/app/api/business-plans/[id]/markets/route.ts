  ...
  try {
    ...
    const run = await openai.beta.threads.runs.create(threadId, {
      ...
      instructions: needsExamples ? 
        ...
        1. Include concrete details about target markets and customer segments
        ...
        5. After presenting examples, ask:
           - "Would you like to use one of these examples as your market description?"
           - "Or would you prefer to modify your current one with some ideas from these examples?"
           - "Or would you like different examples?"
        
        Keep focused on target markets, customer segments, market size, customer needs, market trends, and competitive landscape.
...
/**
 * Extracts structured markets data from conversation
 * @param threadId The thread ID to use
 * @param conversation The conversation history
 * @returns Structured markets data
 */
async function extractMarketsData(threadId: string, conversation: string): Promise<BusinessPlanContent['markets']> {
  try {
    ...
    const structuringPrompt = `Based on this conversation about business markets and customers, 
    extract and structure the following information in JSON format:
    {
      "targetMarket": "Description of the overall target market",
      "marketSize": "Description of the market size and growth potential",
      "customerSegments": ["Array of specific customer segments"],
      "customerNeeds": ["Array of customer needs or problems addressed"],
      "marketTrends": ["Array of current market trends"],
      "competitiveLandscape": "Description of the competitive landscape and position"
    }
    ...
    console.error('Failed to extract structured markets data - no valid JSON found');
    
    // Return a default empty structure rather than throwing an error
    return {
      targetMarket: "Information not available",
      marketSize: "Information not available",
      customerSegments: [],
      customerNeeds: [],
      marketTrends: [],
      competitiveLandscape: "Information not available"
    };
  } catch (error) {
    console.error('Error in extractMarketsData:', error);
    
    // Return a default structure in case of error
    return {
      targetMarket: "Error extracting information",
      marketSize: "Error extracting information",
      customerSegments: [],
      customerNeeds: [],
      marketTrends: [],
      competitiveLandscape: "Error extracting information"
    };
  }
}

/**
 * POST - Main API endpoint handler for chat interactions about markets and customers
 * 
 * Purpose:
 * - Processes incoming chat messages for business plan markets and customers
...
export async function POST(
  ...
  try {
    ...
    // Get structured markets data
    const allMessages = messages.map(m => m.content).join("\n") + "\n" + conversationContent.text.value
    console.log('Extracting structured markets data...');
    const marketsData = await extractMarketsData(threadId, allMessages)
    console.log('Markets data extracted successfully');
    ...
    if (businessPlan) {
      console.log('Updating business plan with markets data...');
      const content = (businessPlan.content || {}) as BusinessPlanContent
      await prisma.businessPlan.update({
        where: { id: params.id },
        data: {
